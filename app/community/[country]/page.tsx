...
export default async function CommunityCountryPage({
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 py-16 text-center">
          ...
          <Link href={submitHref}>
            <Button className="rounded-md!">{submitLabel}</Button>
          </Link>
        </div>
      </div>
    );
  ...
  const communityAvgCaps =
    totalPicks > 0
      ...
      : null;
  const countryStat = countryStats.find((c) => c.code === team.code) ?? null;
  ...
  return (
    <div className="space-y-10">
      <div className="space-y-4">
        <div className="space-y-4">
          ...
          <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link href={submitHref} className="sm:shrink-0">
              <Button size="lg" className="rounded-md!">{submitLabel}</Button>
            </Link>
          </header>
        </div>
        ...
        <div className="space-y-4">
          {formationDef && (
            <div className="community-page-pitch mx-auto w-[81%] max-w-[420px] pb-4 lg:max-w-[560px]">
              <CommunityPitch
        ...
        <div className="space-y-3">
          <SquadAgeBar
            countryAge={stats.averages.age}
            countryEmoji={team.flagEmoji}
            countryCode={team.code}
            globalAvgAge={communityAvgAge}
            userAge={null}
          />
          <SquadCapsBar
            ...
            countryEmoji={team.flagEmoji}
            countryCode={team.code}
            globalCaps={communityAvgCaps}
            userCaps={null}
          />
          <SquadValueBar
            countryValue={stats.averages.marketValueEur}
            countryEmoji={team.flagEmoji}
            countryCode={team.code}
            globalValue={communityAvgValue}
            userValue={null}
          />
        </div>
        </div>
